...
{	
	...
	public class MultipleObjectPooler : ObjectPoolerBase
	{
		/// the list of objects to pool
		public List<MultipleObjectPoolerObject> Pool;
					///A MultipleObjectPooler is a reserve of objects, to be used by a Spawner. When asked, it will return
					/// an object from the pool (ideally an inactive one) chosen based on the pooling method you've chosen.
					...
					/// RandomBetweenObjects will pick one object from the pool, at random, but ignoring its pool size, each
					/// object has equal chances to get picked PoolSizeBased randomly choses one object from the pool, based
					/// on its pool size probability (the larger the pool size, the higher the chances it'll get picked)
					
		/// the chosen pooling method
		public PoolingMethods PoolingMethod = PoolingMethods.RandomPoolSizeBased;
					/// If you set CanPoolSameObjectTwice to false, the Pooler will try to prevent the same object from
					/// being pooled twice to avoid repetition. This will only affect random pooling methods, not ordered
					/// pooling.
